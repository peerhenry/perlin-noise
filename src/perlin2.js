...
function getGradientVectors(makeHash, corners) {
  ...
  // scaling the vec seems to have little effect on the diamond shapedness of noise
  // return corners.map(([i,j]) => {
  //   const rr = 0.5*Math.cos(i+j) + 1
  //   const scale = v => [rr*v[0], rr*v[1]]
  //   const vec = gradientVecs[makeHash(i,j)]
  //   return [rr*vec[0], rr*vec[1]]
  // })
}
